...
export function ComplaintDetail({
  ...
  return (
    <DashboardLayout
      ...
      subtitle={complaint.referenceCode}
      active="complaints"
      onNavigate={(key) => {
        if (key === "complaints") void onBack();
        if (key === "overview") onOverview?.();
      ...
      <div className="flex-1 px-6 py-8 max-w-3xl mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#4A5C99] font-medium">
                    {CATEGORY_LABELS[category]}
                  </span>
                </div>
                <div
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${currentStatus.bg} ${currentStatus.color}`}
                >
              ...
              <h2 className="text-xl font-bold text-[#1E2233] mb-4 leading-snug break-words">
                {complaint.subject}
              </h2>
              <p className="text-[#6B7280] text-sm leading-relaxed mb-5 break-words">
            ...
            {complaint.sla && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h3 className="font-semibold text-[#1E2233] mb-3">
                  Service Level Targets
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div
                    className={`rounded-xl p-3 border ${complaint.sla.firstResponseOverdue ? "bg-red-50 border-red-200 text-red-700" : "bg-gray-50 border-gray-100 text-[#6B7280]"}`}
                  >
                    <p className="font-semibold">First response</p>
                    <p className="mt-1">
                      Due {formatDate(complaint.sla.firstResponseDueAt)}
                    </p>
                    {complaint.sla.firstResponseOverdue && (
                      <p className="font-semibold mt-1">Overdue</p>
                    )}
                  </div>
                  <div
                    className={`rounded-xl p-3 border ${complaint.sla.resolutionOverdue ? "bg-red-50 border-red-200 text-red-700" : "bg-gray-50 border-gray-100 text-[#6B7280]"}`}
                  >
                    <p className="font-semibold">Resolution</p>
                    <p className="mt-1">
                      Due {formatDate(complaint.sla.resolutionDueAt)}
                    </p>
                    {complaint.sla.resolutionOverdue && (
                      <p className="font-semibold mt-1">Overdue</p>
                    )}
                  </div>
                </div>
            ...
            {(complaint.attachments?.length ?? 0) > 0 && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-[#1E2233]">
                    Evidence / Attachments
                  </h3>
                </div>
                <p className="text-xs text-[#6B7280] mb-3">
                  Files are protected and can only be opened by authorized
                  administrators.
                </p>
                  ...
                  {complaint.attachments?.map((attachment) => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const result = await api.getAttachmentUrl(
                            complaint.id,
                            attachment.id,
                          );
                          if (attachmentWindow) {
                            attachmentWindow.opener = null;
                            attachmentWindow.location.href = result.url;
                          } else {
                      ...
                      className="w-full flex items-center gap-3 px-3 py-3 rounded-xl border border-gray-100 hover:bg-[#F7F8FA] text-left disabled:opacity-50"
                    >
                      <Paperclip className="w-4 h-4 text-[#2B3A67] flex-shrink-0" />
                      <span className="text-sm text-[#1E2233] truncate flex-1">
                        {attachment.originalName}
                      </span>
                      <span className="text-xs text-[#6B7280]">
                        {(attachment.size / 1024).toFixed(0)} KB
                      </span>
                      <ExternalLink className="w-3.5 h-3.5 text-[#2B3A67] flex-shrink-0" />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              ...
              <p className="text-xs text-[#6B7280] mb-3">
                This response is visible to the student when they track their
                complaint.
                {!complaint.isAnonymous &&
                  complaint.realEmail &&
                  ` An email will be sent to ${complaint.realEmail} when saved.`}
              </p>
              <textarea
                value={response}
                onChange={(e) => {
            ...
            {complaint.messages?.length > 0 && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  <MessageSquare className="w-4 h-4 text-[#2B3A67]" />
                  <h3 className="font-semibold text-[#1E2233]">
                ...
                <div className="space-y-3">
                  {complaint.messages.map((message, index) => (
                    <div
                      ...
                      className={`rounded-xl p-4 ${message.sender === "student" ? "bg-[#2B3A67]/6 border border-[#2B3A67]/15" : "bg-[#2E9E6B]/6 border border-[#2E9E6B]/20"}`}
                    >
                      ...
                      <p className="text-[#1E2233] text-sm leading-relaxed break-words">
                        {message.body}
                      </p>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              ...
              <p className="text-xs text-[#6B7280] mb-3">
                These notes are for admin use only — never shown to the student.
              </p>
              <textarea
                value={notes}
                onChange={(e) => {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <h3 className="font-semibold text-[#1E2233] text-sm mb-3">
                Submitted By
              </h3>
              {complaint.isAnonymous ? (
                <div className="space-y-2">
                  ...
                  <div>
                    <p className="font-semibold text-purple-700 italic">
                      {complaint.anonymousLabel}
                    </p>
                    <p className="text-xs text-[#6B7280] mt-0.5">
                      Student chose to submit anonymously
                    </p>
                  </div>
                  <div className="bg-purple-50 border border-purple-100 rounded-lg px-3 py-2 text-xs text-purple-600">
                    Identity is hidden per the student's choice. Do not attempt
